...
def find_engine_artifacts_py(root: Path) -> Path:
    candidates = [
        ...
        root / "fantasyai" / "scripts" / "engine_artifacts.py",
    ]
    for c in candidates:
        if c.exists():
            return c
...
def main() -> None:
    ...
    # 1) Hard rule prompt: JSON only (best-effort replace if your file defines PROMPT = f"""...""".strip())
...
'''
    ...
    # 2) Inject deterministic fallback so UI always has usable insights
...
'''.strip()
        ...
        # Insert helper immediately before generate_guarded_llm_summary if present
    ...
    # 3) If parse fails, attach fallback parsed output
