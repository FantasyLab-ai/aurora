...
class TrendAnalyzer:
    ...
    def _fetch_google_trends(self):
        ...
        for t in out:
            ...
            if tl and tl not in seen:
                seen.add(tl)
                uniq.append(tl)
        logger.info("📈 pytrends fetched %d items", len(uniq))
        return uniq
    ...
    def fetch_trends(self):
        ...
        src = "pytrends" if PYTRENDS_AVAILABLE else "fallback"
        if not (trends): src="fallback"
        logger.info("🔎 trend source: %s (after filter=%d)", src, len(filtered or trends))
...
class PromptGeneratorStructured:
    ...
    def _compose_dialogue_with_topic(self, s, topic: str) -> str:
        topic = (topic or "").strip()
        # Reuse tone-specific lines but weave the topic in organically
        bank = {
            "comedy": [
                ("Yeti", "Okay but is anyone else seeing '{topic}' trending right now?"),
                ("Kitty", "We're not chasing clout—unless '{topic}' pays for snacks."),
                ("Bear", "If '{topic}' is real, I'm bringing the big cooler.")
            ],
            "tense": [
                ("Yeti", "If '{topic}' is true, this area could get chaotic fast."),
                ("Kitty", "Record first. Speculate later. '{topic}' has a lot of noise."),
                ("Bear", "I don't like the echoes tonight. '{topic}' feels… off.")
            ],
            "docu": [
                ("Yeti", "Field note: verifying reports on '{topic}'."),
                ("Kitty", "Archiving context so we don't add to confusion on '{topic}'."),
                ("Bear", "Mic levels steady. Keep it factual if we mention '{topic}'.")
            ],
            "awe": [
                ("Yeti", "Wild that '{topic}' lines up with this light."),
                ("Kitty", "The world trends, the forest ignores. '{topic}' can wait."),
                ("Bear", "Focus. Breathe. Let '{topic}' scroll without us for a second.")
            ],
            "surreal": [
                ("Yeti", "If '{topic}' is real, physics has PR now."),
                ("Kitty", "I refuse to let '{topic}' be canon without peer review."),
                ("Bear", "Imagine '{topic}' but with snacks. Now it's plausible.")
            ],
            "cozy": [
                ("Yeti", "We can shoot a calm take on '{topic}'—no hot takes."),
                ("Kitty", "Tea, soft light, light context on '{topic}'."),
                ("Bear", "We post the chill version of '{topic}'.")
            ],
            "light": [
                ("Yeti", "Quick sweep: if '{topic}' fits, we tag it gently."),
                ("Kitty", "No yelling. Just clean audio and '{topic}' context."),
                ("Bear", "Soft voiceover, minimal edits, '{topic}' in lower third.")
            ],
        }.get(s.tone, [])

        # Keep 3–5 lines, avoid same speaker twice in a row
        import random
        lines, last = [], None
        if not bank:
            bank = [("Yeti", f"Logging environment and topic: {topic}"),
                    ("Kitty", "Keep levels clean."),
                    ("Bear", "Proceeding.")]
        ...
        for _ in range(random.randint(3,5)):
            ...
            last = spk
            icon = "🧍‍♂️" if spk == "Yeti" else "🐈" if spk == "Kitty" else "🐻"
            lines.append(f"{icon} {spk}: {text.format(topic=topic)}")
        return "\n\n".join(lines)

    def generate_topic_prompt(self, topic: str, character_set=None):
        """Build a prompt around the raw trending topic (no theme mapping)."""
        if character_set is None:
            character_set = ["Yeti", "Kitty", "Bear"]
        seg = self.segment_counter
        scene = self._pick_scene(topic)
        title = (topic or "Trending Now").strip()[:120]
        environment_text = self._compose_environment(scene)
        character_blocks = self._compose_characters(character_set)
        dialogue_blocks = self._compose_dialogue_with_topic(scene, topic)
        camera_text = self._compose_camera(scene)
        vibe_text = (
            f"FantasyLab.ai hyper-real vlog with Yeti, Kitty, Bear. "
            f"Topic: {title}. Tone: {scene.tone}. "
            f"Stay grounded and truthful; avoid speculation."
        )

        parts = [
            f"🎬 {title}\n📋 Segment {seg} - The Take",
            f"📰 Topic\n{title}",
            f"📍 Environment\n{environment_text}",
            f"🎥 Camera\n{camera_text}",
            f"🧍 Characters\n\n{character_blocks}",
            ...
            "🎛️ Artifacts\n" + "\n".join([
                "Operator's breathing audible in quiet moments",
                "Autofocus wobble when subject crosses highlight",
                "Subtle handheld micro-jitter on pans"
            ]),
            f"🎭 Vibe\n{vibe_text}"
        ]
        final_prompt = "\n\n".join(parts)
...
class PromptScorer:
    """Stubbed scorer. Real scorer lives in scripts/learners/prompt_scorer_impl.py."""
    def __init__(self):
        pass
    def maybe_train_from_csv(self, csv_path=None):
        return False, "external scorer in use"
    def predict(self, prompt_json):
        text = (prompt_json or {}).get("final_prompt", "") or ""
        length = len(text)
        emo = sum(1 for ch in text if ord(ch) >= 0x1F300)
        tags = text.count("#")
        base = max(0.0, 1000 - max(0, length - 600) * 0.5)
        return float(base + emo * 50 + tags * 80)
...
def _with_retry(fn, tries=3, delay=1.2):
    last=None
    for _ in range(max(1,tries)):
        try: return fn()
        except Exception as e:
            last=e; _rt.sleep(delay)
    raise last
...
def generate_veo_video_cdp(prompt_text: str) -> str:
    ...
    try:
        ...
        if '403' in str(e):
            raise RuntimeError('CDP 403: relaunch Chromium with --remote-allow-origins=http://127.0.0.1:9222')
        raise
    ...
    # -- headless hardening --
    try:
        tconn.call("Emulation.setUserAgentOverride", {
            "userAgent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119 Safari/537.36",
            "platform":"Linux","acceptLanguage":"en-US,en;q=0.9"
        })
    except Exception: pass
    for _dom in ("Network.enable","Page.enable","Runtime.enable","DOM.enable"):
        try: tconn.call(_dom if "." in _dom else f"{_dom}.enable", {})
        except Exception: pass
    for _m in ("Browser.setDownloadBehavior","Page.setDownloadBehavior"):
        try: tconn.call(_m, {"behavior":"allow","downloadPath": OUTPUT_DIR})
        except Exception: pass
    ...
    """
    focused = _with_retry(lambda: tconn.call("Runtime.evaluate", {"expression": js_focus, "returnByValue": True}).get("result",{}).get("value"))
    if not focused:
    ...
    """
    clicked = _with_retry(lambda: tconn.call("Runtime.evaluate", {"expression": js_click, "returnByValue": True}).get("result",{}).get("value"))
    if not clicked:
        js_busy = r"""
        (function(){
    ...
    # Ensure downloads allowed again
    for method in ("Browser.setDownloadBehavior", "Page.setDownloadBehavior"):
        try:
    ...
    # Poll for video files (watch outputs/, ~/Downloads, and common profile Downloads)
    profile_dirs = [
        os.getenv("CHROME_USER_DATA_DIR", ""),
        os.path.join(os.path.expanduser("~"), ".chromium-remote"),
        os.path.join(os.path.expanduser("~"), "FantasyAI", ".chrome-profile"),
    ]
    extra_dirs = [d for d in (os.getenv("EXTRA_DOWNLOAD_DIRS","").split(":")) if d]
    watch_dirs = [OUTPUT_DIR, os.path.join(HOME, "Downloads")] + \
                 [os.path.join(d, "Downloads") for d in profile_dirs if d] + extra_dirs

    exts = tuple(x.strip().lower() for x in os.getenv("VIDEO_EXTS", ".mp4,.mov,.webm").split(",") if x.strip())
    logger.info("⏳ Waiting for video in: %s (exts=%s) …", ", ".join([os.path.abspath(d) for d in watch_dirs]), ",".join(exts))
        ...
        for d in watch_dirs:
            try:
                for f in os.listdir(d):
                    fl = f.lower()
                    if any(fl.endswith(ext) for ext in exts) and not fl.endswith(".crdownload"):
                        fp = os.path.join(d, f)
                        if not latest or os.path.getmtime(fp) > os.path.getmtime(latest):
                            latest = fp
            except Exception:
                pass
        if latest:
            try:
                if not os.path.exists(latest + ".crdownload") and os.path.getmtime(latest) >= start_ts - 2:
                    out = latest
                    break
            except Exception:
                pass
    ...
    if not out:
        raise RuntimeError("Timed out waiting for download; check outputs/ and Downloads/ for debug dumps.")
