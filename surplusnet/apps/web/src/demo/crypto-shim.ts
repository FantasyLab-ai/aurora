/**
 * Browser stand-in for `node:crypto`, used only by the self-contained demo
 * build (the engine's ledger hashing and id generation run in-page there).
 * Implements the two calls @surplusnet/core makes: createHash('sha256') with
 * the update().digest('hex') chain, and randomUUID().
 */
